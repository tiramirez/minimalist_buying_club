  ...
  return (
    <div className="p-4 md:shadow md:rounded-lg md:bg-white bg-gray-200 w-full md:w-96">
      ...
      <div className="mt-1 flex w-full justify-between space-x-1">
        <button className={`rounded text-white px-2 text-md md:py-1 md:text-sm ${showMyCart?'bg-violet-400':'bg-zinc-400 text-gray-400'}`} onClick={clickOnMyCart}>{ showMyCart ? "Back to Menu" : "My Cart" }</button>
        <button className={`rounded text-white px-2 text-md md:py-1 md:text-sm ${showMyCart?'bg-red-400 hover:bg-red-600':'bg-red-200'}`} onClick={ showMyCart ? clickOnDeleteCart : clickOnMyCart }>Reset Cart</button>
